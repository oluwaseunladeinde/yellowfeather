...
function fetchListing(paydata){
    $('#preloader').show();
    $('.card-list').empty();
    $.ajax({ url: '/feather/listings/', type: 'GET', data: paydata, dataType: 'json' })
    .done(function(response, textStatus, jqXHR){
        console.log('Got the group ', response);
        ...
        if (data.total > 0){
            ...
            $.each(itemlist, function(index){
                ...
                var el = "<div class=\"card noborder\">\n" +
                    ...
                    "                <span class=\"card__title\"><a href=\"  "+ item.url +"\">"+ item.title +"</a><!-- small class=\"text-muted\">KC420</small --></span>\n" +
