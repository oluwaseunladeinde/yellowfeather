# -*- coding: utf-8 -*-
import re, os, datetime, hashlib, string
...
VALIDATIONS = [
    ('feather.utils.validation_simple', _('One or more characters')),
    ('feather.utils.validation_integer', _('Integer')),
    ('feather.utils.validation_yesno', _('Yes/No')),
    ('feather.utils.validation_decimal', _('Decimal')),
]
...
class Deal(models.Model):
    ...
    price = MoneyField(_('Sale Price'), default=Money(0, NGN), max_digits=12, decimal_places=2)
    active = models.BooleanField(_('Active'), default=False)
...
def send_email(addresses_to, ctx_dict, subject_template, body_template, body_html_template):
  ...
  """
  subject = (getattr(settings, 'REGISTRATION_EMAIL_SUBJECT_PREFIX', '') + render_to_string(subject_template, ctx_dict))
  ...
  email_message = EmailMultiAlternatives(subject, message_txt, from_email, addresses_to)
...
auditlog.register(Deal)
auditlog.register(Location)
auditlog.register(Listing)
auditlog.register(ListingImage)
auditlog.register(Attribute)
auditlog.register(AttributeListing)
